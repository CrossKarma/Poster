...
APP_VERSION = "0.4"  # bump on every update
...
class Channel(db.Model):
    ...
    refresh_token = db.Column(db.Text)
    email = db.Column(db.String(255), default="")   # the Google email behind this channel
    created_at = db.Column(db.DateTime, default=datetime.datetime.utcnow)
...
class Post(db.Model):
    ...
    created_at = db.Column(db.DateTime, default=datetime.datetime.utcnow)
    # The captions Poster wrote for this video, kept so they can be copied
    # again later from the Posted list instead of dying with the queue card.
...
with app.app_context():
    ...
    for ddl in (
        ...
        "ALTER TABLE channel ADD COLUMN IF NOT EXISTS email VARCHAR(255) DEFAULT ''",
        ...
        "ALTER TABLE post ADD COLUMN IF NOT EXISTS tiktok TEXT",
        "ALTER TABLE post ADD COLUMN IF NOT EXISTS instagram TEXT",
        "ALTER TABLE post ADD COLUMN IF NOT EXISTS tweet TEXT",
...
def _api_json_errors(e):
    # API calls must always answer with JSON, never an HTML error page.
    ...
    if request.path.startswith("/api/"):
        return jsonify({"ok": False,
                        "error": "server error (" + type(e).__name__ + "): "
                                 + str(e)[:300] + ". Hit Refresh to retry."}), 500
    ...
    if not u:
        ...
        if linked:
            ...
            if owner:
                ...
                linked.email = info.get("email") or linked.email
...
def list_posts():
    ...
    return jsonify({"ok": True, "posts": [{
        ...
        "tiktok": p.tiktok or "", "instagram": p.instagram or "",
        "tweet": p.tweet or ""
    } for p in rows]})
...
def record_post():
    ...
    p = Post(user_id=u.id,
             ...
             tiktok=str(d.get("tiktok", ""))[:1000],
             instagram=str(d.get("instagram", ""))[:1000],
             tweet=str(d.get("tweet", ""))[:1000])
    db.session.add(p)
    db.session.commit()
...
one format at all, leave that format's week empty rather than inventing times
for it. But if a format has uploads, never leave its week completely blank:
even one or two uploads earn it exactly ONE starter slot, taken from the real
day and Eastern hour those uploads actually went out. When you do that, the
weekWhy line for that slot must say straight out how few uploads it rests on
and that it is a starting point, not a proven pattern, e.g. 'Friday: only two
normal videos exist so far, not enough to call a pattern, but both went out
Friday afternoon so this is the best starting slot until there is more to go
on'.
...
ANALYZE_JOB_MAX_AGE = 8 * 60   # seconds before a silent running job counts as dead


def _analyze_job_key(chan_pk):
    return "ajob_" + str(chan_pk)


def _set_analyze_job(chan_pk, state, error=""):
    val = json.dumps({"s": state, "e": (error or "")[:150],
                      "t": datetime.datetime.utcnow().isoformat()})
    row = db.session.get(Meta, _analyze_job_key(chan_pk))
    if row:
        row.value = val
    else:
        db.session.add(Meta(key=_analyze_job_key(chan_pk), value=val))
    db.session.commit()


def _get_analyze_job(chan_pk):
    row = db.session.get(Meta, _analyze_job_key(chan_pk))
    ...
    try:
        ...
        return None
    try:
        started = datetime.datetime.fromisoformat(d.get("t"))
        d["age"] = (datetime.datetime.utcnow() - started).total_seconds()
    except Exception:
        d["age"] = None
    return d


def _run_analysis(uid, chan_pk, token, qc):
    # The slow part of analysis, run on a background thread so no web
    # request ever outlives the worker timeout. Progress and errors are
    # written to the job row; the page polls /api/analyze/status.
    with app.app_context():
        try:
            u = db.session.get(User, uid)
            act = db.session.get(Channel, chan_pk)
            if not u or not act:
                _set_analyze_job(chan_pk, "error",
                                 "account row went missing, sign out and back in")
                return
            H = {"Authorization": "Bearer " + token}
            lines = []
            recent_ids = []
            ...
            try:
                ...
                pass
            if not lines:
                _set_analyze_job(chan_pk, "error", "could not read your channel")
                return

            previous = ""
            prev = parse_json_col(act.analysis)
            if prev:
            ...
            try:
                ...
                _set_analyze_job(chan_pk, "error",
                                 "the write-up came back malformed ("
                                 + type(e).__name__ + "), hit Refresh to retry")
                return
            ...
            db.session.commit()
            _set_analyze_job(chan_pk, "done")
        except Exception as e:
            try:
                db.session.rollback()
                _set_analyze_job(chan_pk, "error",
                                 "server error (" + type(e).__name__ + "): " + str(e)[:100])
            except Exception:
                pass
...
def analyze():
    # Starts the channel read in the background and returns straight away.
    # A full read takes a minute or two, far past the 30 seconds a web
    # request is allowed to live, so it cannot run inside this request.
    u, err = require_access()
    if err:
        return err
    token = mint_access_token(u)
    if not token:
        ...
        return jsonify({"ok": False,
                        "error": "this account is not fully connected" + who
                                 + ". Use Add account to reconnect it, then try again."}), 409
    act = active_channel(u)
    if not act:
        return jsonify({"ok": False, "error": "no channel on this account yet"}), 409
    job = _get_analyze_job(act.id)
    if (job and job.get("s") == "running"
            and job.get("age") is not None and job["age"] < ANALYZE_JOB_MAX_AGE):
        return jsonify({"ok": True, "started": True, "already": True})
    try:
        qc = int((request.get_json(silent=True) or {}).get("queueCount") or 0)
    except Exception:
        qc = 0
    _set_analyze_job(act.id, "running")
    threading.Thread(target=_run_analysis, args=(u.id, act.id, token, qc),
                     daemon=True).start()
...
def analyze_status():
    ...
    s = job.get("s")
    if s == "running":
        if job.get("age") is not None and job["age"] > ANALYZE_JOB_MAX_AGE:
            return jsonify({"ok": True, "state": "error",
                            "error": "the read was cut off mid-run, hit Refresh to start it again"})
        return jsonify({"ok": True, "state": "running"})
    if s == "error":
        return jsonify({"ok": True, "state": "error",
                        "error": job.get("e") or "analysis failed, hit Refresh to retry"})
    out = parse_json_col(act.analysis) or {}
    if out:
        out = sanitize_analysis(out)
    return jsonify({"ok": True, "state": "done", "analysis": out})
