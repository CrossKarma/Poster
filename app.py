...
def active_channel(u):
    ...
    if act is None:
        act = chans[0]
    # One-time migration: data used to live on the account and bled between
    # channels. Move it onto the channel that was in use, then clear it so
    # every channel is walled off from the others from here on.
    try:
        moved = False
        if u.analysis and not act.analysis:
            act.analysis = u.analysis
            moved = True
        if u.schedule_json and not act.schedule_json:
            act.schedule_json = u.schedule_json
            moved = True
        if u.niche and not act.niche:
            act.niche = u.niche
            moved = True
        if moved or u.analysis or u.schedule_json or u.niche:
            u.analysis = None
