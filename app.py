...
IDEAS_PROMPT = """You are a YouTube content strategist. A creator needs fresh video ideas.

Their niche, in their own words: {niche}

{context}

Give ten specific video ideas that fit this channel and lean on what is trending.
Each idea is one sentence, concrete enough to film this week, under 18 words.
...
def generate_ideas():
    ...
    try:
        ...
        return jsonify({"ok": False, "error": "idea generation failed, try again"}), 502
    saved = []
    for t in ideas[:10]:
