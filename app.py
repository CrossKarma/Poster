...
with app.app_context():
    ...
    # One-time reset: set WIPE_ANALYSIS=1 on Render to clear every saved
    # analysis and schedule so all users regenerate fresh under new rules.
    # Accounts, channels and logins are untouched. Remove the var after deploy.
    if os.environ.get("WIPE_ANALYSIS") == "1":
        for ddl in (
            'UPDATE "user" SET analysis = NULL',
            'UPDATE "user" SET schedule_json = NULL',
        ):
            try:
                db.session.execute(db.text(ddl))
                db.session.commit()
...
def mint_access_token(u):
    ...
    c = active_channel(u)
    refresh = (c.refresh_token if c and c.refresh_token else u.refresh_token)
    if not refresh:
        return None
    try:
...
def me():
    ...
    return jsonify({
        ...
        "analysis": (lambda a: sanitize_analysis(a) if a else a)(parse_json_col(u.analysis)),
    })
...
def auto_niche():
    ...
    u.niche = text[:2000]
    db.session.commit()
    return jsonify({"ok": True, "niche": u.niche})
...
{{
 ...
 "weekSchedule": {{"mon": [], "tue": [], "wed": [], "thu": [], "fri": [], "sat": [], "sun": []}} where each day is a list of 0 or more 24-hour "HH:MM" times in US Eastern time. Derive every time from the actual Eastern hours the creator's better videos went out, shown in the upload list above. NEVER output 00:00 or any hour you did not derive from real data; if you do not have a real hour for a day, either leave that day empty or reuse a proven hour from a similar day. Put more times on days the measured numbers prove are strong, fewer or none on weak days, and cap the total across the week at postsPerDay times 7,
 "weekWhy": ["one short line per day you put times on, naming the day, how it performs, and why you chose it, e.g. 'Friday: your best day, top three videos all went out Friday evening and pulled well above your average'. Empty array if no day-level evidence."],
...
def sanitize_analysis(out):
    ...
    the schedule actually contains. Rebuild weekSchedule from clean times,
    then rewrite weekWhy lines so their times match, dropping lines for any
    day that ended up empty. Also scrub bestTimes/bestHours."""
            ...
            if uploads:
                ...
                if ids:
                    ...
                    vids = http_json("https://www.googleapis.com/youtube/v3/videos"
                                     "?part=snippet,statistics&id=" + ",".join(ids),
                                     headers=H)
                    lines.append("Recent uploads, newest first (title, day of week, published time US Eastern, views, likes, comments):")
                    for v in (vids.get("items") or []):
                        sn = v.get("snippet", {})
                        s = v.get("statistics", {})
                        ...
                        try:
                            ...
                            pass
                        lines.append('- "' + sn.get("title", "")[:80] + '" | went out '
                                     + dow + " at " + hhmm + " | "
                                     + str(s.get("viewCount", "?")) + " views | "
                                     + str(s.get("likeCount", "?")) + " likes | "
    ...
    try:
        if recent_ids:
            ...
            if vrows:
                ...
                for r in vrows[:25]:
                    vid = str(r[0])
                    lines.append('- "' + (title_by_id.get(vid) or vid) + '" | '
